...
<script setup lang="ts">
    ...
    watch(id, (novelId) => {
        importWorkbench.bench(novelId, "Originals");
    }, { immediate: true });

    const bench = computed(() => importWorkbench.bench(id.value, "Originals"));

    // Everything below is a view onto the store entry rather than state of its own, which is what
    // lets the pick, the start chapter and the switch all survive leaving this screen and coming back
    // to it - reloaded or not.
    const rowSelection = computed<Record<string, boolean>>({
        ...
        set: (value: Record<string, boolean>) => {
            importWorkbench.patch(id.value, "Originals", {
    ...
    const startAtNumber = computed<number>({
        ...
        set: (value: number) => importWorkbench.patch(id.value, "Originals", { startAtNumber: value }),
    });

    const replaceExisting = computed<boolean>({
        get: () => bench.value.replaceExisting,
        set: (value: boolean) => importWorkbench.patch(id.value, "Originals", { replaceExisting: value }),
    });
    ...
    const importLabel = computed<string>(() => {
        ...
        return (replaceExisting.value && alreadyImportedCount.value > 0)
            ? `Import ${count}, replacing ${formatCount(alreadyImportedCount.value)}`
            : `Import ${count}`;
    });
    ...
    const statusText = computed(() => {
        ...
        if (listing.value?.state === "Ready") {
            ...
            // Told here, and only while nothing is picked yet: a re-read of a nine-hundred-chapter
            // book buries three new rows at the bottom of the table, and once something is picked
            ...
            return `${formatCount(links.value.length)} chapters from ${siteLabel(listing.value.url)}${when}${since}.`;
        }
    ...
    // The default follows the pick instead of sitting at one: choosing rows six through ten should
    ...
    watch(selectedLinks, (chosen) => {
        if (chosen.length === 0) {
            importWorkbench.patch(id.value, "Originals", { startAtTouched: false });
    ...
    function touchStartAt(): void {
        importWorkbench.patch(id.value, "Originals", { startAtTouched: true });
    }
    ...
    async function runImport(): Promise<void> {
        ...
        importWorkbench.clearPick(id.value, "Originals");
    }
</script>
...
<style scoped lang="scss">
    ...
    .import-url {
        ...
        .picked {
            ...
            // Wide enough for "Import 999, replacing 999" - its longest form - so turning Replace on
            // and off never resizes the button and shifts the bar around it.
            .import {
                flex: none;
                min-width: 13rem;
                justify-content: center;
